...
def check_supabase_resources():
    ...
    try:
        ...
        try:
            print(
                f"  Trying to list storage buckets (with auth token: {bool(auth_token)})"
            )
            ...
            if buckets:
                print(
                    f"  Available buckets: {', '.join([b.get('name', 'unknown') for b in buckets])}"
                )
    ...
    updated_at TIMESTAMPTZ
                    ...
                    print(
                        f"""ALTER TABLE public.{test_table_name} ENABLE ROW LEVEL SECURITY;"""
                    )
