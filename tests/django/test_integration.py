...
class TestSupabaseIntegration:
    ...
    def test_storage_operations(self, client, test_record_data):
        ...
        def cleanup_bucket(bucket_id):
            try:
                storage_service.delete_bucket(
                    bucket_id=bucket_id, auth_token=auth_token, is_admin=True
                )
                print(f"Cleaned up bucket {bucket_id}")
                time.sleep(2)  # Give Supabase time to process the deletion
            except Exception as error:
                print(
                    f"Note: Bucket {bucket_id} already deleted or not found: {str(error)}"
                )
        ...
        try:
            ...
            try:
                ...
                storage_service.delete_bucket(
                    bucket_id=unique_test_bucket, auth_token=auth_token, is_admin=True
                )
                    ...
                    try:
                        ...
                        try:
                            ...
                            storage_service.get_bucket(
                                bucket_id=unique_test_bucket, auth_token=auth_token
                            )
                            print(
                                f"Direct access to bucket {unique_test_bucket} successful"
                            )
                        ...
                        print(
                            f"Found {len(buckets)} buckets: {[b.get('name') for b in buckets]}"
                        )

                        if any(b.get("name") == unique_test_bucket for b in buckets):
                            print(
                                f"Verified bucket {unique_test_bucket} exists in bucket list"
                            )
                            bucket_verified = True
                            break
                        else:
                            print(
                                f"Bucket {unique_test_bucket} not found in bucket list"
                            )
                    except Exception as e:
                        print(
                            f"Attempt {attempt + 1} to verify bucket failed: {str(e)}"
                        )

                    if attempt < max_retries - 1:
                        print(
                            f"Waiting {retry_delay} seconds before retrying verification..."
                        )
                        time.sleep(retry_delay)
                    else:
            ...
            if not bucket_verified:
                print(
                    "Warning: Could not verify bucket exists, but will attempt file operations anyway"
                )
            ...
            for attempt in range(max_retries):
                try:
                    print(
                        f"Attempt {attempt + 1}/{max_retries}: Uploading file to {unique_test_bucket}/{test_file_path}"
                    )
                    ...
                    if attempt > 0 and not upload_success:
                        try:
                            print(
                                "Previous upload attempts failed. Trying to recreate the bucket..."
                            )
                            ...
                            try:
                                ...
                                print(
                                    f"Note: Could not delete bucket: {str(delete_error)}"
                                )
                    ...
                    url = f"{settings.database.SUPABASE_URL}/storage/v1/object/{unique_test_bucket}/{test_file_path}"
                    ...
                    upload_success = True
                    print(
                        f"File uploaded successfully to {unique_test_bucket}/{test_file_path}"
                    )
                    ...
                    if attempt < max_retries - 1:
                        print(
                            f"Waiting {retry_delay} seconds before retrying upload..."
                        )
                        time.sleep(retry_delay)
                    else:
            ...
            for attempt in range(max_retries):
                try:
                    print(
                        f"Attempt {attempt + 1}/{max_retries}: Listing files in bucket {unique_test_bucket}"
                    )

                    # For file listing, we'll use a direct requests approach to have more control
                    import requests

                    url = f"{settings.database.SUPABASE_URL}/storage/v1/object/list/{unique_test_bucket}"
                    ...
                    if file_found:
                        ...
                        print(
                            f"Warning: Uploaded file {test_file_path} not found in file list"
                        )
                        if attempt < max_retries - 1:
                    ...
                    if attempt < max_retries - 1:
                        print(
                            f"Waiting {retry_delay} seconds before retrying file listing..."
                        )
                        time.sleep(retry_delay)
                    else:
            ...
            if upload_success and files_listed and file_found:
                ...
                print(
                    "Skipping download and delete operations due to previous failures"
                )
        except Exception as e:
    ...
    def test_database_operations(self, client, test_record_data):
        ...
        test_table_name = (
            os.getenv("TEST_TABLE_NAME") or f"test_table_{uuid.uuid4().hex[:8]}"
        )
        ...
        try:
            ...
            try:
                result = database_service.fetch_data(
                    table=test_table_name, auth_token=auth_token, limit=1
                )
                print(f"Test table {test_table_name} exists with {len(result)} records")
            except Exception as table_error:
                print(f"Table check error: {str(table_error)}")
                if (
                    "404" in str(table_error)
                    or "Not Found" in str(table_error)
                    or "does not exist" in str(table_error)
                ):
                    table_exists = False
                    print(
                        f"Test table {test_table_name} does not exist, creating it..."
                    )
                    try:
                        ...
                        print(
                            f"Successfully created test table {test_table_name} with RLS policies"
                        )
                        time.sleep(3)  # Give time for the table to be fully available
                        table_exists = True
                    except Exception as create_error:
                        print(f"Failed to create test table: {str(create_error)}")
                        if "function exec_sql() does not exist" in str(
                            create_error
                        ) or "PGRST202" in str(create_error):
                            pytest.skip(
                                "The exec_sql function is not available in this Supabase instance."
                            )
                        else:
                            pytest.fail(
                                f"Could not create test table {test_table_name}. Error: {str(create_error)}"
                            )
                else:
                    raise
            ...
            if table_exists:
                ...
                insert_result = database_service.insert_data(
                    table=test_table_name, data=test_data, auth_token=auth_token
                )
                ...
                print(
                    f"Fetching data from table: {test_table_name} where id = {record_id}"
                )
            ...
            if (
                "test_table_name" in locals()
                and test_table_name
                and not os.getenv("TEST_TABLE_NAME")
            ):
                try:
    ...
    def test_end_to_end_flow(self, client, test_record_data):
        ...
        has_exec_sql = True
        try:
            database_service.call_function(
                "exec_sql", {"query": "SELECT 1"}, auth_token=auth_token
            )
        except Exception as e:
            if "Could not find the function" in str(e) or "PGRST202" in str(e):
                ...
                print(
                    "Database operations that require table creation will be skipped."
                )
                print(
                    "To enable these tests, create the exec_sql function in your Supabase instance."
                )
            else:
                print(
        ...
        try:
            ...
            try:
                ...
                public_url = storage_service.get_public_url(
                    bucket_id=unique_test_bucket, path=test_file_path
                )
                ...
                try:
                    ...
                    assert download_result == test_file_content, (
        ...
        if has_exec_sql:
            ...
            try:
                ...
                database_service.create_test_table(
                    table=unique_test_table, auth_token=auth_token, is_admin=True
                )
                ...
                insert_result = database_service.insert_data(
                    table=unique_test_table, data=test_data, auth_token=auth_token
                )
                ...
                try:
                    ...
                    print(
                        f"Warning: Failed to delete test table: {str(delete_table_err)}"
                    )
